...
namespace NextIteration.SpectreConsole.SelfUpdate.Tests.Pipeline
{
    public sealed class UpdateCheckerVersionTests
    {
        ...
        public void IsNewer_when_numeric_versions_compares_correctly(string current, string latest, bool expected) => Assert.Equal(expected, UpdateChecker.IsNewer(current, latest));
        ...
        public void IsNewer_with_semver_prerelease_compares_correctly(string current, string latest, bool expected) => Assert.Equal(expected, UpdateChecker.IsNewer(current, latest));

        [Theory]
        [InlineData("not-a-version", "1.0.0", false)]
        [InlineData("1.0.0", "not-a-version", false)]
        [InlineData("", "1.0.0", false)]
        public void IsNewer_with_unparseable_versions_returns_false(string current, string latest, bool expected) => Assert.Equal(expected, UpdateChecker.IsNewer(current, latest));

        [Theory]
        [InlineData("1.0.0+abc123", "1.0.0")]
        [InlineData("1.2.3+sha.deadbeef", "1.2.3")]
        [InlineData("1.0.0", "1.0.0")]
        public void StripBuildMetadata_removes_plus_suffix(string input, string expected) => Assert.Equal(expected, UpdateChecker.StripBuildMetadata(input));
        ...
        [InlineData("1.0.0", "1.0.0")]
        [InlineData("", "")]
        public void StripLeadingV_removes_v_prefix(string input, string expected) => Assert.Equal(expected, UpdateChecker.StripLeadingV(input));
        ...
        [InlineData("", "")]
        public void ComputeDefaultSkipEnvVarName_sanitizes_app_name(string appName, string expected) => Assert.Equal(expected, UpdateChecker.ComputeDefaultSkipEnvVarName(appName));
    }
}
